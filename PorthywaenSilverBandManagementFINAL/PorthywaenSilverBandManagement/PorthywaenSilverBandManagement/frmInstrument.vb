...
Public Class frmInstrument
	...
	Sub colours()    'formats and displays data in dgv
		...
		Dim totalRecordsInstrument As Integer

		Try
			dgvInstruments.Rows.Clear() 'clear rows in dgv
	...
	Function validation(serialNo, name, instrument, holderID, serviceDate)
		Dim failed As Boolean   'stops program executing if an input does not conform to validation rules
	...
	Private Sub btnFindAvailable_Click(sender As Object, e As EventArgs) Handles btnFindAvailable.Click
		Dim oneInstrument As instruments    'pointer to structure
		Dim quantity As Integer = 0 'number of instruments found
		Dim empty As String = vbNullChar & vbNullChar & vbNullChar & vbNullChar & vbNullChar    'avoids errors when checking if holderID stored
		...
		Try
			...
			For index = 1 To totalRecords   'search each record for the instrument selected
				FileGet(1, oneInstrument)

				If oneInstrument.instrument.Contains(cmbInstrumentSearch.Text) Then   'search for the same instrument as selected
					If oneInstrument.holderID.Contains(empty) Then  'no id means the instrument is available so add to dgv
						quantity = quantity + 1
		...
		Try
			For currentRow = 1 To dgvInstrumentSearch.RowCount  'format service dates in each row in dgv
				serviceDate = dgvInstruments.Rows(currentRow - 1).Cells(6).Value
				If dgvInstruments.RowCount > 0 Then
					If serviceDate <= red Then  'if date 11 or more months ago, cell is red
					...
					ElseIf serviceDate <= orange And serviceDate > red Then 'if date 10 months ago, cell is orange
					...
					ElseIf serviceDate <= yellow And serviceDate > orange Then  'if date 9 months ago, cell is yellow
						dgvInstrumentSearch.Rows(currentRow - 1).Cells(6).Style.BackColor = Color.Gold
						dgvInstrumentSearch.Rows(currentRow - 1).Cells(6).Style.ForeColor = Color.Black

					ElseIf serviceDate > yellow Then    'if date less than 9 months ago, cell is green
	...
	Private Sub btnFindAll_Click(sender As Object, e As EventArgs) Handles btnFindAll.Click
		...
		Try
			...
			'open file and display record in dgv if contains the instrument
			For index = 1 To totalRecordsInstrument
				FileOpen(2, "players.dat", OpenMode.Random,,, Len(oneMember))
				totalRecordsMember = LOF(2) / Len(oneMember)
				FileGet(1, oneInstrument)

				If oneInstrument.instrument.Contains(cmbInstrumentSearch.Text) Then     'if record contains the instrument
					If oneInstrument.holderID = "" Or oneInstrument.holderID = empty Then
						'if no holderID, add record to dgv with holderID and holderName empty
						dgvInstrumentSearch.Rows.Add(oneInstrument.instrumentID.Trim(), oneInstrument.serialNumber.Trim(), oneInstrument.name.Trim(), oneInstrument.instrument.Trim(),
										oneInstrument.holderID.Trim(), "", oneInstrument.serviceDate)
						quantity += 1
					Else
						'if instrument has a holderID stored, open players file to find the holderName
		...
		Try
			For currentRow = 1 To dgvInstrumentSearch.RowCount
				'format service dates in each row of dgv
				serviceDate = dgvInstruments.Rows(currentRow - 1).Cells(6).Value
				If dgvInstruments.RowCount > 0 Then
					If serviceDate <= red Then  'if more than 11 months ago, cell is red
						dgvInstrumentSearch.Rows(currentRow - 1).Cells(6).Style.BackColor = Color.DarkRed
			...
			FileClose(1)
			If quantity = 0 Then
				MsgBox("No instruments found", vbInformation, "Instruments")
			End If
		Catch ex As Exception  'if fails, give option to retry, else end sub
			MsgBox("Formatting failed", vbExclamation, "Error")
		End Try
	...
	Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
		...
		Try
			...
			'if no records in the file, new id must be the first possible id
	...
	Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
		Try
			...
			'populate structure with inputted data
			If txtHolderID.Text = "" Then   'ensure all empty holderIDs are the same format
				oneInstrument.holderID = empty
			Else
				oneInstrument.holderID = txtHolderID.Text
	...
	Private Sub BtnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
		Try
			...
			Do While Not EOF(1)
				...
				Else
					FileGet(1, oneinstrument)   'if the record to delete, skip
				End If
			Loop
			FileClose(1)
			FileClose(2)
