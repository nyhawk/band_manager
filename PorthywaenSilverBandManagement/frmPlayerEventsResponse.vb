Public Class frmPlayerEventsResponse
	Private Sub playerEventsResponse_Load(sender As Object, e As EventArgs) Handles MyBase.Load
		...
		Dim totalRecordsResponses As Integer = LOF(2) / Len(oneResponse)
		If totalRecordsResponses = 0 Then
			MsgBox("No responses recorded", vbInformation, "Event availibility")
			Exit sub
		End If
		...
		If eventPlayers.Length = 0 Or eventPlayers Is Nothing Then
			MsgBox("No responses recorded for this event", vbInformation, "Event availibility")
			Exit Sub
		End If
